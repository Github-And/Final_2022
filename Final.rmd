---
title: "Covid91 vaccine study FinalSpring2023"
author: "Andrew Valles Tovar"
output: 
  html_document:
    number_sections: true
    toc: true
    toc_depth: 5
date: "`r Sys.Date()`" 
---

```{r include=FALSE,echo=FALSE}
require(tidyverse)
require(tigerstats)
require(forcats)
FinalDat <-read.csv(file="finalFall2022.csv",header=TRUE)
#recode to remove the annoying "X" from the dataframe
FinalData <- FinalDat  %>% select(-one_of("X"))
FinalData <- FinalData %>%
mutate(across(where(is.character), as.factor))
male = filter(FinalData, sex =="M")
female = filter(FinalData, sex =="F")
LGBTQ = filter(FinalData, LGBTQ =="gay")
druggie = filter(FinalData, DrugUser =="yes")
```

# Introduction

A new  preventative vaccine (covid921) is under test in clinical trials. Given the nature of this vaccine it is suspected that the drug may exhibit different effectiveness across the variety of subpopulations tracked. Consider population subgroups by gender( guys and gals), drug use, and sexual orientation. Write a complete report that separately considers this drug for each subgroup and your recommendations as to whether people in the subgroup should take the vaccine. Structure your report in four sections each devoted to one of the four subgroups, use null and alternative hypothesis statements and correct statistical verbage when referencing p-values in conclusions.

## Males

## Bar Chart
```{r}
barchartGC(~infected + treatment,data=male)
barchartGC(~infected + treatment,data=male, type="percent")
```

There are two bar plots here. The one we'll talk about first is the percent bar plot which shows the percent of males in the population that took either the placebo or drug and whether or not they caught COVID. Looking at the percent bar plot, it seems a substantial amount of infected males were under the effects of the placebo. It seems about 65% of infected males took the placebo, and around 30% of took the vaccine. So far it seems the vaccine is effective in preventing the disease for males.

The second plot is almost the same as the one we explained, but instead the y-axis represents a count of the male population instead of percentage. As we can see, only a small amount of males in the population actually contracted COVID-19 compared to the section that didn't catch it. The bar plots going forward from here follow the same explanation.

### Numerical Descriptive Results
```{r}
table1 <- xtabs(~infected + treatment, data=male)
rowPerc(table1)
colPerc(table1)
```
The numerical results here show the exact percentage of males taking either the placebo or vaccine that caught COVID-19 and the percentage of males in the population that contracted the disease. The numerical results going forward from here follow the same explanation.

The row table here shows of those infected with COVID-19, 68% of males had taken the placebo and only 31% took the vaccine. Once again, it seems the vaccine is proving to be effective in preventing the disease for males. It also seems like there were more males that took the vaccine rather than the placebo.

The column table shows the percentage of males in the population that either took the placebo or vaccine and whether or not they caught the disease in their respective category. So nearly 3% of males that took the placebo contracted the disease and a mere 1% of males that took the vaccine were infected.

### Inferential Results
```{r}
chisq.test(table1)
chisqtestGC(table1)
fisher.test(table1)
```
The inferential results here show the exact amount of males in the population and whether or not they caught COVID-19 with the placebo or vaccine.

245 males that had taken the placebo got COVID-19 and 113 of males that took the vaccine also caught the disease. Since there is a difference of 132 males, it seems the vaccine is effective in preventing the disease.

The null hypothesis here is there no relationship between the treatment and males and the alternative hypothesis is there is a relationship between the treatment and males. The P-value indicated by Fisher's test is 0.000000000002364 which is lower than 0.05. The P-value indicated by the Chi-square test is 0.000000000005307 which is also below 0.05. Therefore, we reject the null in favor of the alternative.

## Females

## Bar Chart
```{r}
barchartGC(~infected + treatment,data=female)
barchartGC(~infected + treatment,data=female, type="percent")
```

Strangely, it seems like about 58% of females that contracted COVID-19 had taken the vaccine and about 43% took the placebo. Whereas the vaccine seemed effective on males, it doesn't seem like the case for females. On the frequency graph, only a small amount of females in the population contracted the disease.

### Numerical Descriptive Results
```{r}
table1 <- xtabs(~infected + treatment, data=female)
rowPerc(table1)
colPerc(table1)
```
The numerical results here show of the females infected with COVID-19, 57% took the vaccine and around 43% took the placebo. There also seems to be slightly more placebo participants than the vaccine. Once again it doesn't seem like the vaccine is helping females very much.

For the column table, 1% of females that took the placebo caught the disease and 2% took the vaccine. So while it is a rather small percentage, more females contracted COVID-19 with the vacine.

## LGBTQ



## Druggies


# Overall Results and Conclusions